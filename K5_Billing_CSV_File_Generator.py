...
#          Then obtain the billing information for that project and output the
#          manipulated data to a csv file.
#
# Disclaimer: This script comes with absolutly no guarantees or warranties It has been provided 
#             as an example of how to extract billing data and has not been extensivley tested
...
def create_regional_project_list(region, k5token, userid):
    ...
    try:
        ...
        for item in projects :
            ...
            test = item.get("id")
            name = item.get("name")
            #print "project id = " + str(test)
            #then write it to the opened file
            if "-prj" in name :
                file.write(test+" - default project\n")
            else :
                file.write(test+"\n")
...
def generate_project_bill(k5token, project_id, region, year, month,fileoutput2):
    
    #construct billing URL
    billURL = 'https://billing.gls.cloud.global.fujitsu.com/v1/charges/'+year+'/'+'/'+month+'?project_id='+project_id+'&region_id='+region

    try:
        response = requests.get(billURL,headers={'X-Auth-Token': k5token,'Content-Type': 'application/json','Accept': 'application/json'})

        bill = response.json()
       
        #for each billed item for the project, get product name, usage and charge
        for item in bill :
            product_usage = item.get("usage")
            product_charge = item.get("charge")
            product = item.get("product_name")
            #as product name includes multiple values i.e. both in Japanese and English, extract just the Englih version
            for element in product :
                if "value" in element :
                    language = element.get("lang")
                    if language == "en" :
                        product_name = element.get("value")
                     
            #construct and write a line to the csv file, detailing region, project id, product name, product usage and product charge 
            fileoutput2.write(str(region)+","+str(project_id)+","+str(product_name)+","+str(product_usage)+","+str(product_charge)+"\n")
            #also output to console
            print region, project_id, product_name, product_usage, product_charge
        return True
    except:
        return "Billing Report Error"
...
#initialse a new empty file for writing extracted billing info to for all projects across all regions
fileoutput = open("billingreport.csv","w")
#add header line to file containing coloumn names
fileoutput.write("region, project_id, product_name, product_usage, product_charge\n")
...
#block of code to determine last months number and year
#get todays date
today = datetime.date.today()
#find the first of the month
first = today.replace(day=1)
#find the date for previous month by subtracting 1 day
lastMonth = first - datetime.timedelta(days=1)
#store last months date in YYYYMM format
date = lastMonth.strftime("%Y%m")
#extract last two character from stored date to get month
previousmonth = date[-2:]
#extract first four characters from stored date to get current month
previousmonthsyear = date[:4]

#open file previously created that contains regional project info
fileinput = open("projectlist.txt","r")
#read first line
line = fileinput.readline()
#while not end of file
while line:
    #if line includes region name, extract the region code from it, e.g. uk-1
    if 'region' in line :
        #split the line into words and store in list
        words = line.split()
        #get the region from the sentence, which is the 7th word
        region = words[7]
        #read next line which will be a project id
        line = fileinput.readline()
        #repeat for all lines containing project id, until the next line with 'region' in it is discovered,
        #indicating there are no more project ids to process for the current region.
        while 'region' not in line :
            #extract just project id from line (some lines have text indicating which is the default project)            
            project_id_temp = line.split()
            project_id = project_id_temp[0]

            #call function to extract billing info and write to csv file
            generate_project_bill(token, project_id, region, previousmonthsyear, previousmonth,fileoutput)

            #read next line ready for processing
            line = fileinput.readline()

            #if line is False, then end of file is reached so break out of internal while statement
            if not line :
                #print "breaking"
                break
